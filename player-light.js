...
import { createElementID, isServer, PlayerEvents, PreserveAspectRatio, RendererType } from '@aarsteinmedia/lottie-web/utils';
...
const getMimeFromExt = (ext = '') => {
...
}, isAudio = (asset) => { return !('h' in asset) && !('w' in asset) && 'p' in asset && 'e' in asset && 'u' in asset && 'id' in asset }, parseBase64 = (str) => { return str.slice(Math.max(0, str.indexOf(',') + 1)) }, isBase64 = (str) => {
  if (!str) {
    return false;
  }
...
}, isImage = (asset) => { return 'w' in asset && 'h' in asset && !('xt' in asset) && 'p' in asset }, resolveAssets = async (unzipped, assets) => {
    ...
    toResolve.push(new Promise((resolveAsset) => {
      ...
      if (isServer()) {
        ...
        for (let j = 0; j < jLen; j++) {
          result = result + String.fromCharCode(u8[j]);
        }
  ...
  return manifest;
}, prepareString = (str) => { return str.replaceAll(new RegExp(/"""/, 'g'), '""').replaceAll(/(["'])(.*?)\1/g, (_match, quote, content) => {
...
}, frameOutput = (frame) => { return ((frame ?? 0) + 1).toString().padStart(3, '0') }, handleErrors = (err) => {
  const res = {
...
export { DotLottiePlayer as default, PlayerState, PlayMode, tagName };
