...
export class HTextElement extends TextElement {
  ...
  override buildNewText() {
    ...
    let tParent: null | HTMLElement = null;
    let tCont: null | HTMLElement | SVGSVGElement = null;
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (this.globalData?.fontManager?.chars) {
        ...
        tSpan.textContent = letters[i]?.val ?? null;
        ...
        if (this.isMasked) {
          ...
          const tSpanTranslation = `translate3d(0,${-(documentData?.finalSize || 0) / 1.2}px,0)`;
