...
export class CVTextElement extends TextElement {
  ...
  override buildNewText() {
    ...
    const trackingOffset = documentData.tr * 0.001 * Number(documentData.finalSize);
    let k;
    let xPos = 0;
    let yPos = 0;
    ...
    for (let i = 0; i < length; i++) {
      ...
      const commands = createSizedArray(jLen - 1);
      ...
      for (let j = 0; j < jLen; j++) {
        ...
        const pathNodes = shapes[j]?.ks?.k as ShapePath;
        const pathArr = [];
