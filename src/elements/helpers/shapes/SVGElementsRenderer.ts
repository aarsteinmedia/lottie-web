...
function renderGradient(
  ...
  let stops: SVGStopElement[]; let stop: SVGStopElement;
...
function renderPath(
  ...
  let pathStringTransformed; let shouldRedraw: boolean; let pathNodes: ShapePath | undefined;
  const {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const { lvl: styleLevel } = style ?? { lvl: 0 };
