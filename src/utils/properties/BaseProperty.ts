...
const quaternionToEuler = (out: Vector3, quat: Vector4) => {
  ...
  const qz = quat[2];
  const qw = quat[3];
  const heading = Math.atan2(2 * qy * qw - 2 * qx * qz,
    1 - 2 * qy * qy - 2 * qz * qz);
  const attitude = Math.asin(2 * qx * qy + 2 * qz * qw);
  const bank = Math.atan2(2 * qx * qw - 2 * qy * qz,
    1 - 2 * qx * qx - 2 * qz * qz);
...
const createQuaternion = (values: Vector3): Vector4 => {
...
export abstract class BaseProperty extends DynamicPropertyContainer {
  ...
  getValueAtCurrentTime() {
    ...
    const frameNum = Number(comp?.renderedFrame) - offsetTime;
    const initTime = (keyframes[0]?.t ?? 0) - offsetTime;
    const length = keyframes.length - 1;
    const endTime = (keyframes[length]?.t ?? 0) - offsetTime;
    const { lastFrame } = _caching;
  ...
  interpolateValue(frameNum: number, caching: Caching = {} as Caching) {
    ...
    let len = keyframes.length - 1;
    let shouldInterpolate = true;
    let keyData = keyframes[0] ?? {} as Keyframe;
    let nextKeyData = keyframes[1] ?? {} as Keyframe;
