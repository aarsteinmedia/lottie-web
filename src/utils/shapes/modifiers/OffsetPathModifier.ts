...
const pointDistance = (p1: Vector2, p2: Vector2) =>
...
const offsetSegment = (segment: Segment, amount: number) => {
  let e: Vector2[];
...
const offsetSegmentSplit = (segment: PolynomialBezier, amount: number) => {
  /*
  ...
  */
  ...
  let right: PolynomialBezier;
  let split: PolynomialBezier[];
...
const getIntersection = (a: PolynomialBezier, b: PolynomialBezier) => {
...
const pruneSegmentIntersection = (a: PolynomialBezier[], b: PolynomialBezier[]) => {
  const outa = [...a];
  const outb = [...b];
...
const pruneIntersections = (segments: PolynomialBezier[][]) => {
  let e;
...
export class OffsetPathModifier extends ShapeModifier {
  ...
  processPath(
    ...
    let multiSegments: PolynomialBezier[][] = [];
  ...
  processShapes(_isFirstFrame: boolean) {
    ...
    if (amount !== 0) {
      let shapePaths; let shapeData; let localShapeCollection;
