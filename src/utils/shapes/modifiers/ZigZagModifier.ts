...
const getProjectingAngle = (path: ShapePath, cur: number) => {
  const prevIndex = cur === 0 ? path.length() - 1 : cur - 1;
  const nextIndex = (cur + 1) % path.length();
  const prevPoint = path.v[prevIndex] as Vector2;
  const nextPoint = path.v[nextIndex] as Vector2;
  const pVector = getPerpendicularVector(prevPoint, nextPoint) as Vector2;
...
const zigZagCorner = (
  ...
  const point = path.v[cur % path._length] as Vector2;
  const prevPoint = path.v[cur === 0 ? path._length - 1 : cur - 1] as Vector2;
  const nextPoint = path.v[(cur + 1) % path._length] as Vector2;
  const prevDist =
        pointType === 2
...
const zigZagSegment = (
  ...
  for (let i = 0; i < frequency; i++) {
    ...
    const point = segment.point(t) as Vector2;
...
export class ZigZagModifier extends ShapeModifier {
  ...
  processPath(
    ...
    let segment = shapeSegment(path, 0);
  ...
  processShapes(_isFirstFrame: boolean) {
    ...
    if (amplitude !== 0) {
      let shapeData; let localShapeCollection; let shapePaths;
