...
export abstract class ShapeBaseProperty extends DynamicPropertyContainer {
  ...
  interpolateShape(
    ...
    const kLen = keyPropS.i[0]?.length ?? 0;
  ...
  interpolateShapeCurrentTime() {
    ...
    const initTime = (this.keyframes[0]?.t ?? 0) - this.offsetTime;
    const endTime = (this.keyframes[this.keyframes.length - 1]?.t ?? 0)- this.offsetTime;
    const { lastFrame, lastIndex } = this._caching;
