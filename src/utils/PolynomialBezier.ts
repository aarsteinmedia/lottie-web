...
const splitData = (data: IntersectData) => {
...
const singlePoint = (p: Vector2) => new PolynomialBezier(
  p, p, p, p, false
);
...
export class PolynomialBezier {
  ...
  constructor(
    ...
    const coeffy = polynomialCoefficients(
      p0[1], p1[1], p2[1], p3[1]
    );
  ...
  private _extrema(bez: PolynomialBezier, comp: number) {
    ...
    let max = bez.points[bez.points.length - 1]?.[comp] ?? 0;
