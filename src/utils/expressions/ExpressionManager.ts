...
const hslToRgb = (val: number[]): Vector4 => {
  const h = val[0] ?? 0;
  const s = val[1] ?? 0;
  const l = val[2] ?? 0;
...
function applyEase(
  ...
  let t = tFromProps;
  let val1 = val1FromProps;
  let val2 = val2FromProps;
...
function initiateExpression(
  ...
  let content: undefined | ShapeExpressionInterface;
  let effect: undefined | GroupEffectInterface;
  let randSeed = 0;
  let transform: undefined | TransformExpressionInterface;
  ...
  const outPoint = op / elem.comp.globalData.frameRate;
  const width = sw;
  const height = sh;
  const name = nm;
  let
    anchorPoint: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  let fromComp: null | ((arr: number[]) => Cartesian3D) = null;
  let fromCompToSurface: null | ((arr: number[]) => Cartesian3D) = null;
  let fromWorld: null | ((arr: number[], time: number) => Cartesian3D) = null;
  let loopIn: (type: string, duration: number, wrap?: boolean) => void;
  let loop_in: null | ((type: string, duration: number, wrap?: boolean) => void) = null;
  let loopOut: (type: string, duration: number, wrap?: boolean) => void;
  let loop_out: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  let mask: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  // eslint-disable-next-line prefer-const
  let position: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  // eslint-disable-next-line prefer-const
  let rotation: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  // eslint-disable-next-line prefer-const
  let scale: null | ((type: string, duration: number, flag?: boolean) => void) = null;
  let smooth: null | ((width: number, sample: number) => void) = null;
  let thisComp: null | CompExpressionInterface = null;
  let toComp: null | ((arr: number[], time: number) => void) = null;
  let toWorld: null | ((arr: number[], time: number) => number[]) = null;

  let thisLayer: null | undefined | LayerExpressionInterface;

  let scoped_bm_rt = noOp;
  ...
  let text: TextExpressionInterface | undefined;
  let textIndex: number;
  let textTotal: number;
  let selectorValue: string | undefined;

  const numKeys = property.kf ? k.length : 0;
  const propertyIndex = data.ix;
  const active = !this.data || (this.data as Shape).hd !== true;

  const wiggle = (_freqFromProps: number, amp: number) => {
    // let freq: number
    const { pv: prop } = this;
    const lenWiggle = isArrayOfNum(prop) ? prop.length : 1;
  ...
  const velocityAtTime = this.getVelocityAtTime.bind(this);
  const { frameRate, projectInterface: comp } = elem.comp.globalData;
  ...
  let parent: null | LayerExpressionInterface = null;
...
function linear(
  ...
  let tMax = tMaxFromProps;
  let value1 = value1FromProps;
  let value2 = value2FromProps;
