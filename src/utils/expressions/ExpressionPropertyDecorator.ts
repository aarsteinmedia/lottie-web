...
function loopOut(
  ...
  const currentFrame = comp.renderedFrame ?? 0;
  const lastKeyFrame = keyframes[keyframes.length - 1]?.t ?? 0;
  ...
  if (durationFlag) {
    ...
    firstKeyFrame = keyframes[keyframes.length - 1 - duration]?.t ?? 0;
  ...
  switch (type) {
    ...
    case 'offset': {
      ...
      const endV = this.getValueAtTime(lastKeyFrame / frameRate, 0);
      const current = this.getValueAtTime(((currentFrame - firstKeyFrame) % cycleDuration + firstKeyFrame) / frameRate, 0);
...
function loopIn(
  ...
  const type = typeFromProps ? typeFromProps.toLowerCase() : '';
  const currentFrame = comp.renderedFrame ?? 0;
  const firstKeyFrame = keyframes[0]?.t ?? 0;
  ...
  if (durationFlag) {
    ...
    lastKeyFrame = keyframes[duration]?.t ?? 0;
    cycleDuration = lastKeyFrame - firstKeyFrame;
  }
  let i;
  let len;
  ...
  switch (type) {
    ...
    case 'offset': {
      ...
      const endV = this.getValueAtTime(lastKeyFrame / frameRate, 0);
      const current = this.getValueAtTime((cycleDuration - (firstKeyFrame - currentFrame) % cycleDuration + firstKeyFrame) / frameRate, 0);
...
function smooth(
  ...
  const { frameRate } = comp?.globalData ?? { frameRate: 60 };
...
function getTransformValueAtTime(this: TransformProperty, time: number) {
  ...
  const { mult: aMult = 1 } = this.a ?? { mult: 1 };
  const { mult: sMult = 1 } = this.s ?? { mult: 1 };
  ...
  if (this.sk && this.appliedTransformations < 3) {
    ...
    const skewAxis = this.sa?.getValueAtTime(time) as number | undefined ?? 0;
  ...
  if (this.r && this.appliedTransformations < 4) {
    ...
    const rotationY = Number(this.ry?.getValueAtTime(time));
    const rotationX = Number(this.rx?.getValueAtTime(time));
    const orientation = this.or?.getValueAtTime(time) as number[] | undefined ?? [];
  ...
  if (this.data.p?.s) {
    ...
    const positionY = this.py?.getValueAtTime(time) as number;
