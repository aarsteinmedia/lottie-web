...
function addPropertyDecorator() {
  function loopOut(type, duration, durationFlag) {
    ...
    if (type === 'pingpong') {
      ...
      let lastValue = this.getValueAtTime(
        lastKeyFrame / this.comp.globalData.frameRate,
        0
      )
      let nextLastValue = this.getValueAtTime(
  ...
  function loopIn(type, duration, durationFlag) {
    ...
    if (type === 'pingpong') {
      ...
      let current = this.getValueAtTime(
        (cycleDuration -
          ((firstKeyFrame - currentFrame) % cycleDuration) +
          firstKeyFrame) /
      ...
      let firstValue = this.getValueAtTime(
        firstKeyFrame / this.comp.globalData.frameRate,
        0
      )
      let nextFirstValue = this.getValueAtTime(
  ...
  function getTransformValueAtTime(time) {
    ...
    // / /
    let matrix = this._transformCachingAtTime.v
    matrix.cloneFromProps(this.pre.props)
    if (this.appliedTransformations < 1) {
      let anchor = this.a.getValueAtTime(time)
      matrix.translate(
    ...
    if (this.appliedTransformations < 2) {
      let scale = this.s.getValueAtTime(time)
      matrix.scale(
        scale[0] * this.s.mult,
        scale[1] * this.s.mult,
        scale[2] * this.s.mult
    ...
    if (this.sk && this.appliedTransformations < 3) {
      let skew = this.sk.getValueAtTime(time)
      var skewAxis = this.sa.getValueAtTime(time)
      matrix.skewFromAxis(-skew * this.sk.mult, skewAxis * this.sa.mult)
    }
    if (this.r && this.appliedTransformations < 4) {
      let rotation = this.r.getValueAtTime(time)
      matrix.rotate(-rotation * this.r.mult)
    } else if (!this.r && this.appliedTransformations < 4) {
      let rotationZ = this.rz.getValueAtTime(time)
    ...
    if (this.data.p && this.data.p.s) {
      let positionX = this.px.getValueAtTime(time)
      var positionY = this.py.getValueAtTime(time)
      if (this.data.p.z) {
        let positionZ = this.pz.getValueAtTime(time)
        matrix.translate(
      ...
      let position = this.p.getValueAtTime(time)
      matrix.translate(
  ...
  let getTransformProperty = TransformPropertyFactory.getTransformProperty
  ...
  let propertyGetProp = PropertyFactory.getProp
  ...
  let ShapePropertyConstructorFunction =
    ShapePropertyFactory.getConstructorFunction()
  let KeyframedShapePropertyConstructorFunction =
    ...
    pointOnPath: function (perc, time) {
      ...
      while (i < len) {
        if (accumulatedLength + lengths[i].addedLength > lengthPos) {
          ...
          pt = bez.getPointInSegment(sz.v[endIndex], shapePath.o[initIndex], shapePath.i[endIndex], segmentPerc, lengths[i]);
          break;
        } else {
