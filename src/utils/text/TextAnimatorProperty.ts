...
const addBrightnessToRGB = (color: Vector3, offset: number) => {
  const hsv = rgbToHSV(
...
const addHueToRGB = (color: Vector3, offset: number): Vector3 => {
  const hsv = rgbToHSV(
    color[0] * 255, color[1] * 255, color[2] * 255
  );
...
const addSaturationToRGB = (color: Vector3, offset: number) => {
  const hsv = rgbToHSV(
    color[0] * 255, color[1] * 255, color[2] * 255
  );
...
export class TextAnimatorProperty extends DynamicPropertyContainer {
  ...
  getMeasures(documentData: DocumentData, lettersChangedFlag?: boolean) {
    try {
      ...
      let currentLength = 0;
      let currentPoint: undefined | BezierPoint;
      let segmentLength = 0;
      let shouldMeasure;
      let pointInd = 0;
      let segmentInd = 0;
      let prevPoint: undefined | BezierPoint;
      let points: null | BezierPoint[] = null;
      let segments: BezierData[] = [];
      let partialLength = 0;
      let totalLength = 0;
      let perc;
      let tanAngle;
      let mask: null | ShapeProperty | StarShapeProperty | RectShapeProperty | EllShapeProperty = null;
